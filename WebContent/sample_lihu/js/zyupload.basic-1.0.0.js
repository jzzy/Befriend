...
//					edit             : true,  						      // 是否可以编辑文件（裁剪）
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					html += '			<button type="button" id="fileSubmit" class="upload_submit_btn">上传图片</button>';
					html += '		</div>';
				    ...
				    html += '							<img class="upload_image" src="images/attachimg2.png"/>';
				    html += '						</div>'; 
				    html += '					</a>';
				    html += '				</div>';
					html += '			</div>';
					html += '		</div>';
					html += '		<div class="upload_submit">';
					html += '			<button type="button" style="display: none;" id="fileSubmit" class="upload_submit_btn">上传图片</button>';
					html += '		</div>';
					html += '		<div id="uploadInf" class="upload_inf"></div>';
					html += '	</div>';
					html += '</form>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '	<p id="uploadFailure_'+file.index+'" class="file_failure">上传失败</p>';
